class Game{
    constructor(){

    }
    getState() {
        var gameStateRef = database.ref('gameState');
        gameStateRef.on("value", function (data) {
            gameState = data.val();
        })

    }

    update(state) {
        database.ref('/').update({
            gameState: state
        });
    }
    async start() {
            if (gameState === 0) {
                player = new Player();
                var playerCountRef = await database.ref('playerCount').once("value");
                if (playerCountRef.exists()) {
                    playerCount = playerCountRef.val();
                    player.getCount();
                }
                form = new Form()
                form.display();
            }
    player1 = createSprite(200,500);
    player1.addImage("player1",player_img);
    
    player2 = createSprite(800,500);
    player2.addImage("player2", player_img);
    players=[player1,player2];

        }
    
    play(){
        
                form.hide();

                Player.getPlayerInfo();
                 image(back_img, 0, 0, 1000, 800);
                 var x =100;
                 var y=200;
                 var index =0;
                 drawSprites();
                 for(var plr in allPlayers){
                     
                     index = index+1;
                     x = 500-allPlayers[plr].distance;
                     y=500;
                     
                     players[index -1].x = x;
                     players[index - 1].y = y;
                       
                     if(index === player.index){
                         
                         // to display player name on the basket.
                         fill("black");
                         textSize(25);
                         stroke("black")
                         strokeWeight(1)
                         text(allPlayers[plr].name ,x-55,y+25);

                         
                     }
                    
                     var player1name = database.ref('players/player1/name');
                     player1name.on("value", (data) => {
                     player1name= data.val();
                     })

                     var player2name = database.ref('players/player2/name');
                     player2name.on("value", (data) => {
                     player2name= data.val();
                     })
    
                     
                     //text to display player score.
                     textSize(25);
                     fill("white");
                     text(player1name + ":" + allPlayers.player1.score, 50, 50);
                    text(player2name + ":" + allPlayers.player2.score, 50, 100);
                 
                 }
                
                
                 var player1score = database.ref("players/player1/score");
                 player1score.on("value",(data)=>{
                 player1score = data.val()
                 })

                 var player2score = database.ref("players/player2/score");
                 player2score.on("value",(data)=>{
                 player2score = data.val()
                 })

                if (keyIsDown(RIGHT_ARROW) && player.index !== null&& player.distance >-430) {
                    player.distance -= 10
                    player.update();
                }
                if (keyIsDown(LEFT_ARROW) && player.index !== null&&player.distance <430) {
                    player.distance += 10
                    player.update();
                }
            
                 if (frameCount % 20 === 0) {
                     fruits = createSprite(random(100, 1000), 0, 100, 100);
                     fruits.velocityY = 6;
                     var rand = Math.round(random(1,5));
                     switch(rand){
                         case 1: fruits.addImage("fruit1",fruit1_img);
                         break;
                         case 2: fruits.addImage("fruit1", fruit2_img);
                         break;
                         case 3: fruits.addImage("fruit1", fruit3_img);
                         break;
                         case 4: fruits.addImage("fruit1", fruit4_img);
                         break;
                         case 5: fruits.addImage("fruit1", fruit5_img);
                         break;
                     }
                     fruitGroup.add(fruits);
                     
                 }

                 if(frameCount % 100 === 0 ){
                     worm = createSprite(random(100,1000),0,100,100);
                     worm.velocityY = 5;
                     worm.addImage(worm_img)
                     worm.scale=0.2
                     wormgroup.add(worm)
                 }
                 
                  if (player.index !== null) {
                     // add the condition to calculate the score. and use update to update the values in the database.
                     for (var i = 0; i < fruitGroup.length; i++) {
                        if (fruitGroup.get(i).isTouching(players)) {
                            fruitGroup.get(i).destroy();
                            player.score =player.score+1;
                            player.update();

                        }
                        
                    }
                    if(wormgroup.isTouching(players)){
                        wormgroup.destroyEach()
                        player.score = 0
                        player.update()
                    }
                  }         
         
        
                  if(player1score>19){
                    gameState = 2
                    swal({
                        title: `Congratulations !`,
                        text: player1name +" has won" ,
                        imageUrl:"images/trophy.png",
                        imageSize: "150x150",
                        cancelButtonText: "Ok",
                      });
                }

                if(player2score>19){
                    gameState = 2
                    swal({
                        title: `Congratulations !`,
                        text: player2name +" has won" ,
                        imageUrl:"images/trophy.png",
                        imageSize: "150x150",
                        cancelButtonText: "Ok",
                      });
                }

    }

     
    end(){
       console.log("Game Ended");
    }
}